...
type ClaimResult = {
  success: boolean; message?: string; points?: number; count?: number; limit?: number;
  retryAfter?: number; nextAvailableAt?: string | null; nextResetAt?: string;
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (adType === 'adsgram_task') {
      ...
      const { data, error } = await supabase.rpc('claim_adsgram_task_reward', { p_user_id: appUser.id });
    ...
    const { data, error } = await supabase.rpc('claim_ad_reward', {
      p_user_id: appUser.id, p_provider: provider as AdProvider,
    });
    if (error) throw error;
    ...
    const { data: contests } = await supabase.from('contests').select('id')
      .eq('contest_type', 'ads_watch').eq('is_active', true).lte('starts_at', now).gte('ends_at', now);
    for (const contest of contests || []) {
      const { error: scoreError } = await supabase.rpc('increment_contest_score', {
        p_contest_id: contest.id, p_user_id: appUser.id, p_delta: 1,
      });
      if (scoreError) console.error('Contest score update failed:', scoreError.message);
    }
    return json(result);
  } catch (error) {
