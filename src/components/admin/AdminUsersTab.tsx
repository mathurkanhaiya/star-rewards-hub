...
export default function AdminUsersTab({ users, onBan, onAdjustBalance, message }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {filtered.map(u => (
        <div key={u.id} className="p-3 rounded-xl glass-card" style={{ border: '1px solid hsl(220 20% 15%)' }}>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">
                  ID: {u.telegram_id} • Lv{u.level} • {u.total_points.toLocaleString()} pts
                  {u.balances?.[0] && <> • Bal: {u.balances[0].points.toLocaleString()}</>}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <button
                ...
                style={{ background: 'hsl(45 100% 55% / 0.15)', color: 'hsl(45 100% 60%)' }}
              >
                💰
              </button>
              <button
          ...
          {adjustUserId === u.id && (
            <div className="mt-2 p-2 rounded-lg space-y-2" style={{ background: 'hsl(220 25% 6%)', border: '1px solid hsl(45 100% 55% / 0.2)' }}>
              <div className="flex gap-2">
                <input type="number" value={adjustAmount} onChange={e => setAdjustAmount(e.target.value)}
                  placeholder="Points (+/-)" className="flex-1 px-2 py-1.5 rounded-lg text-xs outline-none"
                  style={{ background: 'hsl(220 25% 5%)', border: '1px solid hsl(220 20% 20%)', color: 'hsl(210 40% 95%)' }} />
                <input value={adjustReason} onChange={e => setAdjustReason(e.target.value)}
                  placeholder="Reason" className="flex-1 px-2 py-1.5 rounded-lg text-xs outline-none"
                  style={{ background: 'hsl(220 25% 5%)', border: '1px solid hsl(220 20% 20%)', color: 'hsl(210 40% 95%)' }} />
              </div>
              <button onClick={() => {
                const pts = parseInt(adjustAmount);
                if (!isNaN(pts) && adjustReason) {
                  onAdjustBalance(u.id, pts, adjustReason);
          ...
          <div className="text-xs text-muted-foreground mt-1">{new Date(u.created_at).toLocaleDateString()}</div>
        </div>
      ))}
    </div>
  );
