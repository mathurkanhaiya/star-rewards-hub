...
export default function HomePage(){
  ...
  useEffect(()=>{const t=setInterval(()=>setEnergy(prev=>{if(prev>=MAX_ENERGY)return MAX_ENERGY;const next=Math.min(MAX_ENERGY,prev+REGEN_PER_SEC*(fastActive?FAST_REGEN_MULT:1));localStorage.setItem("energy",String(next));localStorage.setItem("lastEnergyTime",String(Date.now()));return next;}),1000);return()=>clearInterval(t);},[MAX_ENERGY,REGEN_PER_SEC,FAST_REGEN_MULT,fastActive]);
  useEffect(()=>{const t=setInterval(()=>{setX2SecsLeft(p=>Math.max(0,p-1));setFastSecsLeft(p=>Math.max(0,p-1));setAdCooldown(p=>Math.max(0,p-1));setDropCooldown(p=>Math.max(0,p-1));},1000);return()=>clearInterval(t);},[]);
  useEffect(()=>{if(!farmStart)return;const t=setInterval(()=>{const elapsed=Date.now()-farmStart;const pct=Math.min(100,(elapsed/FARM_DURATION_MS)*100);setFarmProgress(pct);if(elapsed>=FARM_DURATION_MS){setFarmReady(true);setFarmProgress(100);setFarmTimeLeft("Ready!");clearInterval(t);}else{const rem=Math.ceil((FARM_DURATION_MS-elapsed)/1000);setFarmTimeLeft(`${Math.floor(rem/60)}:${(rem%60).toString().padStart(2,"0")}`);}},500);return()=>clearInterval(t);},[farmStart,FARM_DURATION_MS]);
  ...
  return <><style>{CSS}</style><div className="hp-root">
    ...
    <div className="hp-tap-card"><div className="hp-tap-header"><div className="hp-tap-title">⚡ TAP <span>TO EARN</span></div><div className="hp-energy-pill">⚡ {Math.floor(energy)}/{MAX_ENERGY}</div></div><div className="hp-tap-center"><div className="hp-tap-btn-wrap"><div className="hp-tap-ripple"/><div className="hp-tap-ripple"/><div className="hp-tap-ripple"/><button ref={tapBtnRef} className="hp-tap-btn" onClick={handleTap} disabled={energy<1}><span className="hp-tap-btn-emoji">🪙</span><span className="hp-tap-btn-sub">+{TAP_REWARD*(x2Active?2:1)} PTS</span></button>{floatPts.map(f=><div key={f.id} className="hp-float-pts" style={{left:f.x,top:f.y}}>+{f.val}</div>)}</div><div className="hp-energy-wrap"><div className="hp-energy-labels"><span>ENERGY</span><span style={{color:energyColor}}>{energy>=MAX_ENERGY?"⚡ FULL":fastActive?`⚡ FAST ×${FAST_REGEN_MULT}`:`+${(REGEN_PER_SEC*60).toFixed(1)}/min`}</span></div><div className="hp-energy-track"><div className="hp-energy-fill" style={{width:`${energyPct}%`,background:`linear-gradient(90deg,${energyColor}80,${energyColor})`}}/></div>{energy<1&&<div className="hp-regen-label">⏳ Recharging...</div>}</div></div><div className="hp-boost-row"><button className="hp-boost-btn x2" onClick={()=>!x2Active&&showX2Ad()} disabled={x2Active}><div className="hp-boost-label">⚡ 2× TAP</div>{x2Active?<div className="hp-boost-timer">{fmtBoost(x2SecsLeft)}</div>:<div className="hp-boost-sub">Watch ad • {X2_DURATION_SEC}s</div>}</button><button className="hp-boost-btn fast" onClick={()=>!fastActive&&showFastAd()} disabled={fastActive}><div className="hp-boost-label">🔋 FAST ×{FAST_REGEN_MULT}</div>{fastActive?<div className="hp-boost-timer">{fmtBoost(fastSecsLeft)}</div>:<div className="hp-boost-sub">Watch ad • {FAST_DURATION_SEC}s</div>}</button></div></div>

    <div className="hp-drop-card"><div className="hp-drop-header"><div className="hp-drop-title">🎁 Daily Drop</div><div className="hp-drop-streak">🔥 {dropStreak>0?`${Math.min(dropStreak,DROP_MAX_DAYS)} Day${dropStreak>1?"s":""}`:"New"}</div></div>{dropLoading?<div className="hp-drop-loading"><div className="hp-drop-spin"/>Loading...</div>:<div className="hp-drop-days">{DAILY_DROP.map((d,i)=>{const claimed=i<todayDayIdx||(i===todayDayIdx&&dropClaimedToday),locked=i>todayDayIdx;return <div key={d.day} className={`hp-drop-day ${claimed?"claimed":""} ${locked?"locked":""}`} style={i===todayDayIdx&&!dropClaimedToday?{borderColor:d.color}:undefined}>{claimed&&<div className="hp-drop-check">✓</div>}<div className="hp-drop-pts" style={{color:claimed?"#4ade80":locked?"rgba(255,255,255,.25)":d.color}}>{d.pts}</div><div className="hp-drop-dlabel">{d.label}</div></div>;})}</div>}<button className={`hp-drop-btn ${dropClaimedToday?"claimed":dropCooldown>0||dropLoading?"cooldown":"claim"}`} onClick={handleClaimDrop} disabled={dropBtnDisabled}>{dropClaiming?"CLAIMING...":dropClaimedToday?"✅ Claimed Today!":dropLoading?"Loading...":dropCooldown>0?`⏳ Available in ${dropCooldown}s`:`🎁 CLAIM +${DAILY_DROP[todayDayIdx]?.pts||DROP_BASE} PTS`}</button></div>

    <div className="hp-farm-card"><div className="hp-farm-top"><div className="hp-farm-icon">🌾</div><div className="hp-farm-info"><div className="hp-farm-title">FARMING</div><div className={`hp-farm-sub ${isFarming||farmReady?"live":""}`}>{farmReady?"✦ Ready to claim!":isFarming?`⏱ ${farmTimeLeft} remaining`:`Start Farming → ${num("farm_duration_minutes",15)} min → +${FARM_REWARD} pts`}</div></div><div className="hp-farm-badge">+{FARM_REWARD} PTS</div></div><div className="hp-farm-prog-labels"><span>{farmReady?"Complete!":isFarming?"Farming...":"Idle"}</span><span>{Math.round(farmProgress)}%</span></div><div className="hp-farm-track"><div className="hp-farm-fill" style={{width:`${farmProgress}%`,background:farmReady?"linear-gradient(90deg,#ffbe00,#f59e0b)":"linear-gradient(90deg,#4ade80,#22d3ee)"}}/></div>{farmReady?<button className="hp-farm-btn claim" onClick={handleFarmClaim} disabled={farmClaiming}>🚜 CLAIM NOW</button>:isFarming?<button className="hp-farm-btn wait" disabled>🌾 FARMING... {farmTimeLeft}</button>:<button className="hp-farm-btn start" onClick={handleFarmStart} disabled={farmClaiming}>🌾 START FARMING</button>}</div>

    <div className="hp-ad-card"><div className="hp-ad-top"><div className="hp-ad-icon">🎬</div><div className="hp-ad-info"><div className="hp-ad-title">WATCH ADS</div><div className="hp-ad-sub">{adsToday>=AD_MAX_PER_DAY?"✅ Daily limit reached":`${adsToday} / ${AD_MAX_PER_DAY} today`}</div></div><div className="hp-ad-badge">+{AD_REWARD} PTS</div></div><div className="hp-ad-prog-track"><div className="hp-ad-prog-fill" style={{width:`${Math.min(100,(adsToday/AD_MAX_PER_DAY)*100)}%`,background:"linear-gradient(90deg,#ffbe00,#f59e0b)"}}/></div><button className={`hp-ad-btn ${adsToday>=AD_MAX_PER_DAY||adCooldown>0?"ghost":"gold-btn"}`} onClick={handleWatchAd} disabled={adLoading||adCooldown>0||adsToday>=AD_MAX_PER_DAY}>{adLoading?"LOADING...":adsToday>=AD_MAX_PER_DAY?"✅ COME BACK TOMORROW":adCooldown>0?`⏳ ${adsToday===0?"READY":"NEXT AD"} IN ${adCooldown}s`:`🎬 WATCH AD +${AD_REWARD} PTS`}</button></div>

    <div className="hp-tabs"><button className={`hp-tab ${activeTab==="earn"?"active":""}`} onClick={()=>setActiveTab("earn")}>Earn</button><button className={`hp-tab ${activeTab==="history"?"active":""}`} onClick={()=>setActiveTab("history")}>History</button></div>
    {activeTab==="earn"?<div><div style={{textAlign:"center",padding:"14px 0 12px",fontFamily:"'Orbitron',monospace",fontSize:9,letterSpacing:"3px",color:"rgba(255,255,255,.1)",textTransform:"uppercase"}}>✦ More Ways to Earn ✦</div><AdsgramTask blockId="task-25198"/></div>:<div>{transactions.length===0?<div className="hp-tx-empty">No transactions yet</div>:transactions.map(t=><div key={t.id} className="hp-tx"><div className="hp-tx-icon">{txIcon(t.type)}</div><div className="hp-tx-body"><div className="hp-tx-label">{txLabel(t.type)}</div><div className="hp-tx-sub">Points earned</div></div><div className="hp-tx-pts">+{t.points}</div></div>)}</div>}
  </div></>;
}
