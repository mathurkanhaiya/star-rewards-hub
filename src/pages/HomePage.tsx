...
export default function HomePage(){
  const {user,refreshBalance,settings:appSettings}=useApp();
  const {t}=usePreferences();
  const [settings,setSettings]=useState<Record<string,string>>(()=>appSettings);
  ...
  const MAX_ENERGY=Math.max(1,num("tap_max_energy",1));
  const REGEN_PER_SEC=num("tap_energy_regen_per_hour",0)/3600;
  ...
  const FARM_REWARD=Math.max(0,num("farm_reward_points",0));
  const AD_MAX_PER_DAY=Math.max(1,num("max_ads_per_day",1));
  const AD_REWARD=Math.max(0,num("ad_reward_points",0));
  const AD_COOLDOWN_SEC=Math.max(0,num("ad_cooldown_seconds",0));
  const AD_INIT_DELAY_SEC=Math.max(0,num("ad_init_delay_seconds",0));
  const DROP_COOLDOWN_SEC=Math.max(0,num("daily_drop_cooldown_seconds",0));
  const DROP_BASE=Math.max(0,num("daily_drop_base",0));
  const DROP_INCREMENT=Math.max(0,num("daily_drop_increment",0));
  ...
  const [energy,setEnergy]=useState<number>(()=>{const saved=Number(localStorage.getItem("energy"));const live=Number(appSettings.tap_max_energy);return Number.isFinite(saved)&&saved>0?saved:Number.isFinite(live)&&live>0?live:0;});
  ...
  const [dropStreak,setDropStreak]=useState(0),[dropClaimedToday,setDropClaimedToday]=useState(false),[dropClaiming,setDropClaiming]=useState(false),[dropLoading,setDropLoading]=useState(true),[dropCooldown,setDropCooldown]=useState(0);
  const dropClaimingRef=useRef(false);
  const [adsToday,setAdsToday]=useState(0),[adCooldown,setAdCooldown]=useState(0),[adLoading,setAdLoading]=useState(false);
  const isAdRunning=useRef(false);

  useEffect(()=>{if(Object.keys(appSettings).length)setSettings(appSettings);},[appSettings]);
  const reloadSettings=useCallback(async()=>{const s=await getSettings();setSettings(s);},[]);
  useEffect(()=>{const id=setInterval(reloadSettings,5000);const f=()=>reloadSettings();window.addEventListener("focus",f);return()=>{clearInterval(id);window.removeEventListener("focus",f);};},[reloadSettings]);
  useEffect(()=>{setEnergy(p=>p<=0?MAX_ENERGY:Math.min(MAX_ENERGY,Math.max(0,p)));setDropCooldown(p=>p>0?p:DROP_COOLDOWN_SEC);setAdCooldown(p=>p>0?p:AD_INIT_DELAY_SEC);},[MAX_ENERGY,DROP_COOLDOWN_SEC,AD_INIT_DELAY_SEC]);
  ...
  const onX2Reward=useCallback(()=>{const exp=Date.now()+X2_DURATION_SEC*1000;saveBoost("boostX2Exp",exp);setX2SecsLeft(X2_DURATION_SEC);triggerHaptic("success");showMsg(`⚡ 2x ${X2_DURATION_SEC}s`);},[X2_DURATION_SEC]);
  const{showAd:showX2Ad}=useRewardedAd(onX2Reward);
  const onFastReward=useCallback(()=>{const exp=Date.now()+FAST_DURATION_SEC*1000;saveBoost("boostFastExp",exp);setFastSecsLeft(FAST_DURATION_SEC);triggerHaptic("success");showMsg(`🔋 ×${FAST_REGEN_MULT} ${FAST_DURATION_SEC}s`);},[FAST_DURATION_SEC,FAST_REGEN_MULT]);
  const{showAd:showFastAd}=useRewardedAd(onFastReward);

  async function handleClaimDrop(){if(!user||dropClaimedToday||dropClaiming||dropLoading||dropCooldown>0||dropClaimingRef.current)return;dropClaimingRef.current=true;setDropClaiming(true);try{const dayIndex=Math.min(dropStreak,DAILY_DROP.length-1);const pts=DAILY_DROP[dayIndex]?.pts||DROP_BASE;await reward(pts,"daily_drop",`🎁 Daily Drop Day ${dayIndex+1}: +${pts} pts`);setDropClaimedToday(true);setDropStreak(p=>p+1);showMsg(`+${pts} pts 🎁`);}catch(e){const msg=(e as Error).message;if(/already|claimed|once per day|today/i.test(msg))setDropClaimedToday(true);showMsg(msg);}finally{setDropClaiming(false);dropClaimingRef.current=false;}}

  const onFarmStartReward=useCallback(()=>{const now=Date.now();setFarmStart(now);setFarmProgress(0);setFarmReady(false);localStorage.setItem("farmStart",String(now));triggerHaptic("impact");showMsg(`🌾 ${t('farming')}`);},[t]);
  ...
  return <><style>{CSS}</style><div className="hp-root">
    ...
    {activeTab==="earn"?<div><div style={{textAlign:"center",padding:"14px 0 12px",fontFamily:"'Orbitron',monospace",fontSize:9,letterSpacing:"3px",color:"rgba(255,255,255,.1)",textTransform:"uppercase"}}>✦ {t('earn')} ✦</div><AdsgramTask blockId="task-25198"/></div>:<div>{transactions.length===0?<div className="hp-tx-empty">{t('noTransactions')}</div>:transactions.map(tx=><div key={tx.id} className="hp-tx"><div className="hp-tx-icon">{txIcon(tx.type)}</div><div className="hp-tx-body"><div className="hp-tx-label">{txLabel(tx.type)}</div><div className="hp-tx-sub">{t('pointsEarned')}</div></div><div className="hp-tx-pts">+{tx.points}</div></div>)}</div>}
  </div></>;
}
