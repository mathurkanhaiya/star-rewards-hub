...
/* ── Symbol definitions with rarity colors ── */
const SYMBOLS = [
...
.cf-star {
  ...
  border-radius: 50%;
  background: #fff;
  animation: cfTwinkle var(--dur) ease-in-out infinite var(--delay);
}
@keyframes cfTwinkle {
...
.cf-card-scene.flipped .cf-card-inner {
  transform: rotateY(180deg);
}
.cf-card-scene.shaking {
  animation: cfCardShake 0.3s ease-in-out;
}
@keyframes cfCardShake {
...
/* Card back */
.cf-card-back {
  background: linear-gradient(145deg, #1a0e2e, #12082a);
...
.cf-card-back::before {
  ...
  border-radius: 10px;
  background: 
    repeating-linear-gradient(45deg, rgba(167,139,250,0.03) 0px, rgba(167,139,250,0.03) 1px, transparent 1px, transparent 8px),
    repeating-linear-gradient(-45deg, rgba(167,139,250,0.03) 0px, rgba(167,139,250,0.03) 1px, transparent 1px, transparent 8px);
}
.cf-card-back-symbol {
  font-size: 28px;
...
.cf-card-back-shine {
  ...
  animation: cfShineBack 4s ease-in-out infinite;
}
...
.cf-idle-rune {
  ...
  text-align: center;
  margin-bottom: 14px;
  animation: cfRuneFloat 4s ease-in-out infinite;
  filter: drop-shadow(0 0 20px rgba(167,139,250,0.5));
}
@keyframes cfRuneFloat {
...
.cf-match-label {
  ...
  padding: 6px 24px;
  border-radius: 12px;
  animation: cfMatchPop 0.5s cubic-bezier(0.34,1.56,0.64,1);
}
@keyframes cfMatchPop {
...
.cf-pts-num {
  ...
  line-height: 1;
  letter-spacing: 2px;
  animation: cfPtsPop 0.5s 0.15s cubic-bezier(0.34,1.56,0.64,1) both;
}
@keyframes cfPtsPop {
...
.cf-btn::after {
  ...
  animation: cfBtnShine 3s ease-in-out infinite;
}
...
.cf-loading-dots span:nth-child(2) { animation-delay: 0.2s; }
.cf-loading-dots span:nth-child(3) { animation-delay: 0.4s; }
  ...
  return (
    ...
    >
      <div className="cf-card-inner">
        ...
        {/* Front */}
        <div
          className="cf-card-face cf-card-front"
          style={symbol ? {
...
export default function CardFlipPage() {
  ...
  const flipCard = async (i: number) => {
    ...
    if (next.every(Boolean)) {
      ...
      if (unique === 1) {
        pts = 100; match = 'triple'; color = '#fbbf24';
        triggerHaptic('success');
      } else if (unique === 2) {
        pts = 50; match = 'pair'; color = '#a78bfa';
        triggerHaptic('success');
      } else {
        pts = 15 + Math.floor(Math.random() * 10);
        match = 'none'; color = '#94a3b8';
        triggerHaptic('error');
      }
            ...
            {phase === 'locked' && (
              <>
                ...
                <div className="cf-idle-text">
                  Watch a short ad to receive<br />three mystical cards
                </div>
                <button className="cf-btn cf-btn-purple" onClick={handleUnlock}>
                  📺 &nbsp;Watch Ad to Play
                </button>
              </>
            )}
              ...
              <>
                ...
                {phase === 'picking' && (
                  <div className="cf-tap-hint">
                    {flippedCount === 0 ? '✦ Choose a card to reveal ✦' : flippedCount < 3 ? `${3 - flippedCount} card${3 - flippedCount > 1 ? 's' : ''} remaining` : 'Revealing...'}
                  </div>
                )}
                ...
                {phase === 'result' && (
                  <>
                    ...
                    <button className="cf-btn cf-btn-gold" onClick={handleUnlock}>
                      📺 &nbsp;Play Again
                    </button>
                  </>
                )}
