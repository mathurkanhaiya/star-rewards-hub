...
/* Heat color for number buttons based on proximity to target (revealed post-guess) */
function heatColor(n: number, target: number, guessed: boolean): { bg: string; border: string; color: string; shadow: string } {
  if (!guessed) return {
    bg: 'rgba(34,211,238,0.06)', border: 'rgba(34,211,238,0.2)',
    color: 'rgba(255,255,255,0.7)', shadow: 'none',
  };
    ...
    <div style={{ position: 'relative', width: '140px', height: '140px', margin: '0 auto 20px' }}>
      ...
      {[1, 2, 3].map(i => (
        <div key={i} style={{
          ...
          inset: `${i * 16}px`,
          borderRadius: '50%',
          border: `1px solid ${color}${active ? '30' : '18'}`,
...
.ng-radar-bg::after {
  ...
  background:
    ...
    radial-gradient(ellipse 60% 40% at 80% 90%, rgba(6,182,212,0.04) 0%, transparent 50%);
}
...
.ng-idle-icon {
  ...
  text-align: center;
  margin-bottom: 14px;
  animation: ngFloat 3s ease-in-out infinite;
...
.ng-num-btn {
  ...
  font-family: 'Black Ops One', sans-serif;
  font-size: 20px;
  transition: transform 0.12s, box-shadow 0.2s, background 0.3s, border-color 0.3s, color 0.3s;
  position: relative;
  overflow: hidden;
}
...
.ng-num-btn.target {
  animation: ngReveal 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards;
}
...
.ng-compare-num {
  ...
  font-size: 56px;
  line-height: 1;
  animation: ngReveal 0.4s cubic-bezier(0.34,1.56,0.64,1);
}
.ng-compare-arrow {
  font-size: 20px;
...
.ng-tier-pill {
  ...
  letter-spacing: 2px;
  margin-bottom: 16px;
  animation: ngReveal 0.4s 0.1s cubic-bezier(0.34,1.56,0.64,1) both;
}
...
.ng-pts {
  text-align: center;
  margin-bottom: 24px;
  animation: ngPtsPop 0.5s 0.2s cubic-bezier(0.34,1.56,0.64,1) both;
}
.ng-pts-num {
  font-family: 'Black Ops One', sans-serif;
  font-size: 68px;
...
.ng-btn::after {
  ...
  animation: ngShine 3s ease-in-out infinite;
}
...
.ng-loading-dots span {
  display: inline-block;
  width: 8px; height: 8px;
  border-radius: 50%;
  background: #22d3ee;
  margin: 0 4px;
  animation: ngDot 1.2s ease-in-out infinite;
}
.ng-loading-dots span:nth-child(2) { animation-delay: 0.2s; }
.ng-loading-dots span:nth-child(3) { animation-delay: 0.4s; }
...
export default function NumberGuessPage() {
  ...
  const handleGuess = async (n: number) => {
    ...
    await supabase.functions.invoke('log-ad', {
      body: { userId: user.id, adType: 'number_guess', rewardGiven: tier.pts },
    });
    refreshBalance();
  };
    ...
    <>
      ...
      <div className="ng-root">
        ...
        <div className="ng-content">
          ...
          {/* Sonar ring — shown during guessing */}
          {phase === 'guessing' && (
            <SonarRing active={true} color="#22d3ee" />
          )}
          {phase === 'result' && tier && (
            <SonarRing active={true} color={tier.color} />
          )}
          ...
          <div className="ng-arena">
            ...
            {phase === 'locked' && (
              <>
                ...
                <div className="ng-idle-text">
                  Watch a short ad to reveal<br />a hidden number (1–10)
                </div>
            ...
            {phase === 'result' && tier && guess !== null && (
              <>
                ...
                <button className="ng-btn ng-btn-gold" onClick={handleUnlock}>
                  📺 &nbsp;Play Again
                </button>
              </>
            )}
          ...
          <div className="ng-table">
            ...
            {TIERS.map((t, i) => (
              <div key={i} className="ng-table-row">
                <div className="ng-table-diff" style={{
