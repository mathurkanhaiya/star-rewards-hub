...
function AppContent(){
 ...
 return <><style>{CSS}</style><div className="app-shell"><Header/><div className="top-shortcuts"><button className={`leader-btn ${currentPage==='leaderboard'?'active':''}`} onClick={()=>setCurrentPage(currentPage==='leaderboard'?'home':'leaderboard')}><Trophy/>Leaderboard</button></div>{render()}<BottomNav currentPage={currentPage} onNavigate={setCurrentPage}/></div></>;
}
